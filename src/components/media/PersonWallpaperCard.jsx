...
export default function PersonWallpaperCard({ wallpaper }) {
  const url = wallpaper.download_url || wallpaper.image_url
  const filename = `person-wallpaper-${wallpaper.id}.jpg`
  ...
  return (
    <motion.div whileHover={{ scale: 1.03 }} className="rounded-3xl bg-dark-card overflow-hidden shadow-2xl w-[220px] sm:w-[240px] lg:w-[260px] flex-shrink-0">
      <div className="aspect-[16/9] w-full">
        <ProgressiveImage src={wallpaper.image_url} alt="Person wallpaper" className="h-full" imgClassName="w-full h-full object-cover" />
      </div>
      <div className="p-2 sm:p-3">
        {wallpaper.resolution && <p className="text-[10px] sm:text-xs text-gray-400 mb-1.5">{wallpaper.resolution}</p>}
